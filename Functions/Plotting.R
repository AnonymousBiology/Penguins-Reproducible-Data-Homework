...
# Function to create scatter plot for the relationship between culmen depth and body mass in just Adelie penguins 
create_adelie_culmen_depth_body_mass_scatter_plot <- function(data) {
  plot <- ggplot(data, aes(x = culmen_depth_mm, y = body_mass_g, color = sex)) +
    ...
    labs(title = "Scatter Plot of Culmen Depth vs. Body Mass in Adelie Penguins",
         x = "Culmen Depth (mm)",
         y = "Body Mass (g)") +
    theme_bw()
...
create_adelie_culmen_depth_body_mass_scatter_plot_regression <- function(data) {
  plot <- ggplot(data, aes(x = culmen_depth_mm, y = body_mass_g, color = sex)) +
    ...
    labs(title = "Scatter Plot of Culmen Depth vs. Body Mass in Adelie Penguins with Regression Line",
         x = "Culmen Depth (mm)",
         y = "Body Mass (g)") +
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5, size = 12)) +  # Center and adjust title size
    theme(axis.title = element_text(size = 10))  # Adjust axis title size
